...
describe('Attack Command', () => {
  ...
  describe('Basic functionality', () => {
    it('should attack a character by full name - hit scenario', async () => {
      // Mock Math.random for D20 system - return 0.95 to get roll of 20 (guaranteed hit)
      // Roll formula: Math.floor(Math.random() * 20) + 1
      // 0.95 * 20 = 19, Math.floor(19) + 1 = 20
      jest.spyOn(Math, 'random').mockReturnValue(0.95);
      
      // Create a character in the room with health and specific stats
      // Player character (default stats around 10) vs Enemy with low dex (easy to hit)
      await db.run(
        'INSERT INTO characters (game_id, name, type, current_room_id, is_dead, max_health, current_health, strength, dexterity) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [gameId, 'Goblin Warrior', 'enemy', playerRoomId, 0, 10, 10, 10, 5]
      );
      ...
      expect((controller as any).lastDisplayMessage).toContain('You attack the Goblin Warrior. The Goblin Warrior takes 2 damage.');
    ...
    it('should attack a character by full name - miss scenario', async () => {
      // Mock Math.random for D20 system - return 0.05 to get roll of 1 (low roll)
      // Roll formula: Math.floor(Math.random() * 20) + 1
      // 0.05 * 20 = 1, Math.floor(1) + 1 = 2 (still very low)
      // Use 0.0 to guarantee roll of 1
      jest.spyOn(Math, 'random').mockReturnValue(0.0);
      
      // Create a character with high dexterity (hard to hit)
      // Player with default stats vs enemy with high dex = likely miss
      await db.run(
        'INSERT INTO characters (game_id, name, type, current_room_id, is_dead, max_health, current_health, strength, dexterity) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [gameId, 'Goblin Warrior', 'enemy', playerRoomId, 0, 10, 10, 10, 18]
      );
      ...
      expect((controller as any).lastDisplayMessage).toContain('You attack the Goblin Warrior, but miss!');
    ...
    it('should attack a character by partial name', async () => {
      ...
      // Create a character in the room with health and low dex
      await db.run(
        'INSERT INTO characters (game_id, name, type, current_room_id, is_dead, max_health, current_health, strength, dexterity) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [gameId, 'Crypt Keeper', 'npc', playerRoomId, 0, 10, 10, 10, 5]
      );
      ...
      expect((controller as any).lastDisplayMessage).toContain('You attack the Crypt Keeper. The Crypt Keeper takes 2 damage.');
    ...
    it('should be case-insensitive', async () => {
      ...
      await db.run(
        'INSERT INTO characters (game_id, name, type, current_room_id, is_dead, max_health, current_health, strength, dexterity) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [gameId, 'Giant Spider', 'enemy', playerRoomId, 0, 10, 10, 10, 5]
      );
      ...
      // Check output - should show damage but not kill
      expect((controller as any).lastDisplayMessage).toContain('You attack the Giant Spider. The Giant Spider takes 2 damage.');
    ...
    it('should work with NPCs', async () => {
      ...
      // Create an NPC in the room with health and low dex
      await db.run(
        'INSERT INTO characters (game_id, name, type, current_room_id, is_dead, max_health, current_health, strength, dexterity) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [gameId, 'Friendly Merchant', 'npc', playerRoomId, 0, 10, 10, 10, 5]
      );
      ...
      // Check output - should show damage but not kill
      expect((controller as any).lastDisplayMessage).toContain('You attack the Friendly Merchant. The Friendly Merchant takes 2 damage.');
  ...
  describe('Error handling', () => {
    ...
    it('should prevent attacking dead characters', async () => {
      ...
      // Check output
      expect((controller as any).lastDisplayMessage).toContain('The Dead Goblin is already dead.');
    });
    ...
    it('should handle multiple characters with partial match', async () => {
      ...
      // Create multiple goblins with health and low dex
      await db.run(
        'INSERT INTO characters (game_id, name, type, current_room_id, is_dead, max_health, current_health, strength, dexterity) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [gameId, 'Goblin Warrior', 'enemy', playerRoomId, 0, 10, 10, 10, 5]
      );
      await db.run(
        'INSERT INTO characters (game_id, name, type, current_room_id, is_dead, max_health, current_health, strength, dexterity) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [gameId, 'Goblin Archer', 'enemy', playerRoomId, 0, 10, 10, 10, 5]
      );
  ...
  describe('Integration with other systems', () => {
    ...
    it('should work with characters created by AI generation', async () => {
      ...
      // Check output - should show damage but not kill
      expect((controller as any).lastDisplayMessage).toContain('You attack the Ancient Guardian. The Ancient Guardian takes 2 damage.');
    ...
    it('should work with hostile characters', async () => {
      ...
      // Check output - should show damage but not kill
      expect((controller as any).lastDisplayMessage).toContain('You attack the Hostile Bandit. The Hostile Bandit takes 2 damage.');
  ...
  describe('Character death state', () => {
    it('should reduce health and kill character when health reaches 0', async () => {
      ...
      expect((controller as any).lastDisplayMessage).toContain('The Test Enemy dies from your attack!');
    ...
    it('should not attack already dead characters', async () => {
      ...
      expect((controller as any).lastDisplayMessage).toContain('The Dead Zombie is already dead.');
    });
    ...
    it('should allow attacking multiple different characters', async () => {
      ...
      expect((controller as any).lastDisplayMessage).toContain('The First Orc dies from your attack!');
      ...
      expect((controller as any).lastDisplayMessage).toContain('The Second Orc dies from your attack!');
  ...
  describe('Damage system', () => {
    it('should deal exactly 2 damage per attack when hit', async () => {
      ...
      expect((controller as any).lastDisplayMessage).toContain('You attack the Sturdy Troll. The Sturdy Troll takes 2 damage.');
    ...
    it('should deal no damage when attack misses', async () => {
      // Mock Math.random for D20 system - guaranteed miss (roll of 1)
      jest.spyOn(Math, 'random').mockReturnValue(0.0);
      
      // Create a character with high dexterity (hard to hit)
      const characterId = (await db.run(
        'INSERT INTO characters (game_id, name, type, current_room_id, is_dead, max_health, current_health, strength, dexterity) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [gameId, 'Lucky Troll', 'enemy', playerRoomId, 0, 20, 20, 10, 18]
      )).lastID as number;

      // Execute attack (should miss)
      await (controller as any).processCommand('attack Lucky Troll');
      expect((controller as any).lastDisplayMessage).toContain('You attack the Lucky Troll, but miss!');
    ...
    it('should handle mixed hit and miss attacks correctly', async () => {
      // Mock sequence for D20 system: hit(20), miss(1), hit(20), miss(1)
      const randomValues = [0.95, 0.0, 0.95, 0.0];
      let callCount = 0;
      jest.spyOn(Math, 'random').mockImplementation(() => randomValues[callCount++]);
      
      // Create a character with balanced dexterity for predictable results
      const characterId = (await db.run(
        'INSERT INTO characters (game_id, name, type, current_room_id, is_dead, max_health, current_health, strength, dexterity) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [gameId, 'Test Target', 'enemy', playerRoomId, 0, 20, 20, 10, 15]
      )).lastID as number;

      // First attack (hit) - should reduce health from 20 to 18
      await (controller as any).processCommand('attack Test Target');
      expect((controller as any).lastDisplayMessage).toContain('You attack the Test Target. The Test Target takes 2 damage.');
      ...
      await (controller as any).processCommand('attack Test Target');
      expect((controller as any).lastDisplayMessage).toContain('You attack the Test Target, but miss!');
    ...
    it('should kill character when health reaches exactly 0', async () => {
      ...
      await (controller as any).processCommand('attack bandit');
      expect((controller as any).lastDisplayMessage).toContain('The Wounded Bandit dies from your attack!');
    ...
    it('should kill character when health would go below 0', async () => {
      ...
      expect((controller as any).lastDisplayMessage).toContain('The Nearly Dead Rat dies from your attack!');
    ...
    it('should show multiple attacks required for high health characters', async () => {
      ...
      expect((controller as any).lastDisplayMessage).toContain('You attack the Tough Knight. The Tough Knight takes 2 damage.');
      ...
      await (controller as any).processCommand('attack knight');
      expect((controller as any).lastDisplayMessage).toContain('The Tough Knight dies from your attack!');
  ...
  describe('Weapon damage integration', () => {
    ...
    test('should deal additional damage when player has equipped weapon', async () => {
      ...
      // Mock random for D20 system - guaranteed hit
      jest.spyOn(Math, 'random').mockReturnValue(0.95);

      // Create a character with known health and low dexterity (easy to hit)
      const characterId = (await db.run(
        'INSERT INTO characters (game_id, name, type, current_room_id, is_dead, max_health, current_health, sentiment, strength, dexterity) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [gameId, 'Test Orc', 'enemy', playerRoomId, 0, 10, 10, 'hostile', 10, 5]
      )).lastID as number;
    ...
    test('should use base damage when no weapon equipped', async () => {
      ...
      const characterId = (await db.run(
        'INSERT INTO characters (game_id, name, type, current_room_id, is_dead, max_health, current_health, sentiment, strength, dexterity) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [gameId, 'Test Goblin', 'enemy', playerRoomId, 0, 10, 10, 'hostile', 10, 5]
      )).lastID as number;
