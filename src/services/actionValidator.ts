...
export class ActionValidator {
  ...
  private async checkItemCurses(
    ...
    try {
      const curse = await this.db.get<ItemCurse>(
      ...
      const preventedActions = JSON.parse(curse.prevents_actions);
      
      if (preventedActions.includes(action)) {
        return {
          allowed: false,
          reason: curse.curse_message,
          hint: curse.removal_condition || 'This curse cannot be easily removed.',
          blocker: curse
        };
      }
  ...
  private async getActionConditions(
    ...
    try {
      return await this.db.all<ActionCondition>(
        'SELECT * FROM action_conditions WHERE action_type = ? AND ((entity_type = ? AND entity_id = ?) OR entity_type = ?) ORDER BY priority ASC',
        [action, 'room', context.roomId, 'global']
      );
    } catch (error) {
  ...
  private async evaluateItemInRoomCondition(
    ...
    try {
      ...
      const itemInRoom = await this.db.get<any>(
        'SELECT id FROM room_items WHERE room_id = ? AND item_id = ?',
        [context.roomId, requiredItemId]
      );
  ...
  private async evaluateItemRequiredCondition(
    ...
    try {
      ...
      const characterItem = await this.db.get<any>(
        'SELECT id FROM character_inventory WHERE character_id = ? AND item_id = ?',
        [context.characterId, requiredItemId]
      );
  ...
  private async evaluateItemForbiddenCondition(
    ...
    try {
      ...
      const characterItem = await this.db.get<any>(
        'SELECT id FROM character_inventory WHERE character_id = ? AND item_id = ?',
        [context.characterId, forbiddenItemId]
      );
